...
2. **Poll cycle** — Every 15 s with active WebSocket clients, every 5 min when idle.  Per account:
   ...
   - *Vault account* — reads the access token from `ccswitch-vault / email`, probes `/v1/messages`, stores the result.  **Never refreshes proactively.**  Refresh only fires reactively — on probe-401, on swap-step-0.5 (incoming account on promotion), or on an explicit user Revalidate click.  The CLI cannot see this entry, so the refresh is race-free.
...
- Click **Revalidate** (secondary button on stale cards).  It runs a single refresh attempt on-demand (with 3× Keychain-persist retry if Anthropic rotated but the local Keychain write blipped).  If the refresh succeeds — which it typically does for transient 400s that have since cleared — the stale flag disappears and the account is back in rotation.  If it fails with a terminal OAuth code (`invalid_grant` / `invalid_client` / `unauthorized_client` / `unsupported_grant_type` / `invalid_scope` / `invalid_request_error` / `authentication_error`) the tokens really are dead; use Re-login.  Transient failures increment the same escalation counter the poll loop uses — so repeated clicks against a genuinely broken refresh endpoint will eventually trip the escalation and mark the account stale with the transient-ladder reason, which is intentional (prevents user-driven DoS of the refresh endpoint).
...
**Account card DOES NOT show stale — self-healing after server-side token invalidation**
- CCSwitch now refreshes vault tokens **reactively** (only on probe 401), not proactively.  When Anthropic's server invalidates a stored access_token before its claimed expiry (OAuth 2.1 rotation semantics, server-side revocation, edge-proxy 401), the poll loop's probe hits 401, the reactive path calls the refresh endpoint under a per-email lock, and retries the probe once with the fresh access_token.  If the retry succeeds the account silently recovers — no stale_reason written, no UI change.  Only if the retry still 401s OR the refresh itself returns a terminal OAuth error does the account get marked stale.
- This replaces the pre-April-16 behavior where CCSwitch proactively refreshed vault tokens ~1/hour, each rotation a potential broken-chain trigger that invalidated the entire token family.
...
**ToS positioning.**  CCSwitch operates strictly as **credential storage rotation** — it does not proxy, intercept, or redirect API traffic.  Traffic is generated by the native Claude Code CLI using the native macOS Keychain entry.  Under Anthropic's Feb 2026 ToS clarification ("OAuth tokens from subscription plans may not be used in third-party tools / Agent SDK"), CCSwitch stays architecturally safe because it doesn't route traffic through a third-party server; it only swaps which account's credentials sit in the standard Keychain slot at any moment.  The reactive-only refresh policy (April 16) further reduces the tool's observable fingerprint by ~10× (one refresh per probe-401 event instead of ~1/hour per vault).  However, Anthropic can server-side invalidate tokens at their discretion; if terms tighten further or server-side fingerprinting flags this usage pattern, tokens may be revoked.  Evaluate your organisation's risk tolerance.
